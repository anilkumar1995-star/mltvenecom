    ...
    <div class="container-fluid">
        <nav aria-label="breadcrumb" class="mb-3">
            <ol class="breadcrumb small mb-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">Specification Tables</li>
            </ol>
        </nav>
        ...
        <div class="toolbar d-flex justify-content-between align-items-center mb-3">
            <div class="d-flex gap-2 align-items-center">
                ...
                <form id="searchForm" method="GET" action="{{ route('admin.spec.tables.index') }}">
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('admin.spec.tables.index') }}" class="btn btn-outline-secondary btn-sm">Reload</a>
                <a href="{{ route('admin.spec.tables.create') }}" class="btn btn-primary btn-sm"><i class="fas fa-plus me-1"></i> Create</a>
            </div>
        </div>
        ...
        <form id="bulkForm" action="{{ route('admin.spec.tables.bulk_delete') }}" method="POST">
            @csrf
            <div class="card">
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            @foreach($tables as $table)
                                <tr>
                                    <td><input class="row-check" type="checkbox" name="ids[]" value="{{ $table->id }}" /></td>
                                    <td>{{ $table->id }}</td>
                                    <td><a href="{{ route('admin.spec.tables.edit', $table) }}">{{ $table->name }}</a></td>
                                    <td>{{ $table->description ?? '—' }}</td>
                                    <td>{{ isset($table->groups) ? $table->groups->count() : '—' }}</td>
                                    <td>{{ optional($table->created_at)->format('Y-m-d') }}</td>
                                    <td class="text-end">
                                        <a href="{{ route('admin.spec.tables.edit', $table) }}" class="btn btn-sm btn-primary btn-icon-square"><i class="fas fa-edit"></i></a>
                                        <form action="{{ route('admin.spec.tables.destroy', $table) }}" method="POST" style="display:inline-block" class="confirm-delete-form">
                                            @csrf
                                            @method('DELETE')
                ...
                <div class="card-footer">
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="small text-muted d-flex align-items-center">
                            ...
                            <span>Show from 1 to {{ $tables->count() }} in</span>
                            <span class="record-pill ms-2">{{ $tables->total() }}</span>
