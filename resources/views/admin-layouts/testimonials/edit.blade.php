...
<div class="page-wrapper">
                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">
                                    <h1 class="mb-0 d-inline-block fs-6 lh-1">Edit "{{ $testimonial->name }}"</h1>
                                </li>
                            </ol>
                        </nav>
                    </div>
    ...
    <main class="page-body page-content">
        <div class="container-xl">
            ...
            <form method="POST" action="{{ route('admin.testimonials.update', $testimonial->id) }}" accept-charset="UTF-8" id="botble-testimonial-form" class="js-base-form dirty-check" novalidate="novalidate" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                            ...
                            <div class="card-body">
                                <div class="form-body">
                                    ...
                                    <div class="mb-3 position-relative">
                                        ...
                                        <input class="form-control @error('name') is-invalid @enderror" data-counter="120" placeholder="Name" required="required" name="name" type="text" value="{{ old('name', $testimonial->name) }}" id="name">
                                        @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                    </div>

                                    <div class="mb-3 position-relative">
                                        <label class="form-label form-label" for="company">Company</label>
                                        <input class="form-control @error('company') is-invalid @enderror" data-counter="120" placeholder="Company (optional)" name="company" type="text" value="{{ old('company', $testimonial->company) }}" id="company">
                                        @error('company') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                    </div>
                                    
                                    <div class="mb-3 position-relative">
                                        <label class="form-label form-label required" for="content">Content</label>
                                        <textarea class="form-control @error('content') is-invalid @enderror" rows="5" placeholder="Testimonial content" id="content" name="content" required="required">{{ old('content', $testimonial->content) }}</textarea>
                                    ...
                                    <div style="width: 8rem" class="preview-image-wrapper mb-1">
                                        <div class="preview-image-inner position-relative d-inline-block">
                                            <a class="image-box-actions custom-image-trigger" href="javascript:void(0)">
                                                <img id="testimonial-image-preview" class="preview-image default-image img-fluid rounded border" data-default="{{ asset('vendor/core/core/base/images/placeholder.png') }}" src="{{ $testimonial->image ? \App\Helpers\ImageHelper::getImageUrl() . $testimonial->image : asset('vendor/core/core/base/images/placeholder.png') }}" onerror="this.src='{{ asset('vendor/core/core/base/images/placeholder.png') }}'" alt="Preview image" style="max-height: 150px; width: auto; object-fit: cover;">
                                                <span class="image-picker-backdrop"></span>
                                            </a>
                                            <button class="btn btn-pill btn-icon btn-sm image-picker-remove-button p-0 position-absolute top-0 end-0 bg-white" id="testimonial-remove-image-btn" style="{{ $testimonial->image ? 'display: flex;' : 'display: none;' }} transform: translate(30%, -30%); z-index: 10;" type="button" title="Remove image">
