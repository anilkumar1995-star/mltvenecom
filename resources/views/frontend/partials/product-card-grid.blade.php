...
<div class="tp-product-item-2 mb-20" style="background: #fff; border-radius: 12px; height: 100%; border: 1px solid #f0f0f0; overflow: hidden; position: relative; transition: all 0.3s ease; {{ $isOutOfStock ? 'opacity: 0.7;' : '' }}">
    ...
    <div class="tp-product-content-2" style="padding: 6px; border-top: 1px dashed #eee;">
        ...
        <div class="mb-1">
            ...
            @if($product->is_on_sale && $product->price > $product->final_price)
                <div style="font-size: 10px; color: #34a853; font-weight: 700;">₹{{ number_format($product->price - $product->final_price) }} OFF</div>
            @endif
        </div>
        ...
        <div class="d-flex align-items-center gap-2">
            @if($product->reviews_avg > 0)
                <div class="d-flex align-items-center" style="background: #f3fdf5; color: #34a853; font-size: 10px; font-weight: 700; padding: 1px 5px; border-radius: 4px;">
                    <i class="fas fa-star me-1" style="font-size: 8px;"></i>
                    {{ number_format($product->reviews_avg, 1) }}
                </div>
                <div style="font-size: 10px; color: #999;">({{ number_format($product->reviews_count) }})</div>
            @endif
        </div>
    </div>
</div>
