    ...
    <div class="container-fluid">
        ...
        <div class="toolbar d-flex justify-content-between align-items-center mb-3">
            <div class="d-flex gap-2 align-items-center">
                ...
                <form id="searchForm" method="GET" action="{{ route('admin.spec.groups.index') }}">
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('admin.spec.groups.index') }}" class="btn btn-outline-secondary btn-sm">Reload</a>
                <a href="{{ route('admin.spec.groups.create') }}" class="btn btn-primary btn-sm"><i class="fas fa-plus me-1"></i> Create</a>
            </div>
        </div>
        ...
        <form id="bulkForm" action="{{ route('admin.spec.groups.bulk_delete') }}" method="POST">
            @csrf
            <div class="card">
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            @foreach($groups as $group)
                                <tr>
                                    <td><input class="row-check" type="checkbox" name="ids[]" value="{{ $group->id }}" /></td>
                                    <td>{{ $group->id }}</td>
                                    <td><a href="{{ route('admin.spec.groups.edit', $group) }}">{{ $group->name }}</a></td>
                                    <td>{{ $group->description ?? '—' }}</td>
                                    <td>{{ optional($group->created_at)->format('Y-m-d') }}</td>
                                    <td class="text-end">
                                        <a href="{{ route('admin.spec.groups.edit', $group) }}" class="btn btn-sm btn-primary btn-icon-square"><i class="fas fa-edit"></i></a>
                                        <form action="{{ route('admin.spec.groups.destroy', $group) }}" method="POST" style="display:inline-block" class="confirm-delete-form">
                                            @csrf
                                            @method('DELETE')
                ...
                <div class="card-footer">
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="small text-muted d-flex align-items-center">
                            ...
                            <span>Show from 1 to {{ $groups->count() }} in</span>
                            <span class="record-pill ms-2">{{ $groups->total() }}</span>
        ...
        <script>
            (function(){
                ...
                // confirmation modal handlers (modal markup is in HTML below)
                ...
                function showConfirm(callback){
                    console.log('[spec-groups] showConfirm called');
                    console.trace();
                    pendingConfirmAction = callback;
                    if(modalEl) {
                        console.log('[spec-groups] removing d-none from modal');
                        modalEl.classList.remove('d-none');
                        modalEl.setAttribute('aria-hidden', 'false');
                    } else {
                        console.warn('[spec-groups] modalEl is null');
                    }
                }
