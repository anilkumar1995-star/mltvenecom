    ...
    public static function apiLog($url, $modal, $txnid, $header, $request, $response)
    {
        try {
            $apiresponse = Apilog::create([
                ...
                "modal" => $modal,
                "txnid" => $txnid,
                "header" => is_array($header) || is_object($header) ? json_encode($header) : $header,
                "request" => is_array($request) || is_object($request) ? json_encode($request) : $request,
    ...
    public static function curl($url, $method = 'GET', $parameters, $header, $log = "no", $modal = "none", $txnid = "none")
    {
        ...
        if ($log != "no") {
            ...
            Apilog::create([
                ...
                "txnid" => $txnid,
                "header" => is_array($header) || is_object($header) ? json_encode($header) : $header,
                "request" => is_array($parameters) || is_object($parameters) ? json_encode($parameters) : $parameters,
                "response" => is_array($response) || is_object($response) ? json_encode($response) : $response
            ]);
        }
